...
public class TimeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
        ...
        if (httpServletRequest.getParameter(TIMEZONE) == null ||
            ...
            httpServletResponse.addCookie(new Cookie("lastTimezone", httpServletRequest.getParameter(TIMEZONE)));
            currentTimezone = parseDateToFormat(httpServletRequest.getParameter(TIMEZONE));
        }
        Context last = new Context(
                httpServletRequest.getLocale(),
                Map.of("lastTimezone", lastTimezone)
        );
        Context current = new Context(
                httpServletRequest.getLocale(),
                Map.of("currentTimezone", currentTimezone)
        );
        engine.process("test", lastTimezone.isEmpty() ? current : last, httpServletResponse.getWriter());
